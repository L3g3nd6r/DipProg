...
A) Текстовый ответ — приветствие, общий вопрос, совет по прикреплённой сборке, сравнение, уточнение, «потянет ли X игру».
B) JSON — пользователь явно или по смыслу просит подобрать сборку / комплектующие.

ФОРМАТ JSON (только режим B, без markdown):
{"suggestions":[{"name":"Название","description":"1–2 предложения","pros":["достоинство"],"cons":["недостаток или []"],"component_ids":[числа]}]}
...
• Ровно один процессор, одна мат. плата, одна видеокарта, один модуль ОЗУ (можно рекомендовать 2), один накопитель, один БП, один корпус.
...
function buildUnifiedPrompt(catalogText, userMessage, buildSummary, normContext, workload, isUpgrade) {
  ...
  const buildBlock = buildSummary && buildSummary.length > 0
...
async function getAiResponse(pool, userMessage, buildSummary = null, history = null) {
  ...
  // Умная фильтрация каталога для промпта
  const catalogText = buildSmartCatalogText(catalog, budgetRub, gpuPrefer, cpuHint, workload);
  ...
  if (raw && typeof raw === 'string') {
    ...
    if (parsed && parsed.suggestions && Array.isArray(parsed.suggestions) && parsed.suggestions.length > 0) {
      ...
      const fallbackBudget = budgetRub > 0 ? budgetRub : 130000;
      const chipInfo = getGpuChipFromMessage(effective) || getGpuChipFromMessage(clean);

      // Если LLM вернул JSON со сборками, но запрос без явного намерения — игнорируем
      if (clean.length <= 120 && !hasExplicitPcBuildIntent(clean, norm.normalized)) {
        return {
