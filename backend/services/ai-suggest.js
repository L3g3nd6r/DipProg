...
/**
 * Один лучший вариант под указанный бюджет (~92% суммы — максимум пользы без перерасхода).
 */
function generateSingleOptimalVariant(catalog, maxBudget, gpuPrefer, cpuHint, workload, userMsg) {
  const budget = Math.max(30000, Math.floor(maxBudget * 0.92));
  let build = pickCompatibleBuild(catalog, budget, gpuPrefer, cpuHint, userMsg);
  if (!build) build = pickCompatibleBuild(catalog, maxBudget, gpuPrefer, cpuHint, userMsg);
  if (!build) return null;
  const verified = finalizeAiComponentIds(catalog, build, maxBudget, gpuPrefer, cpuHint, userMsg);
  if (!verified) return null;
  const pc = buildProsCons(catalog, verified.ids, workload);
  return {
    label: 'Оптимальная сборка',
    pct: 0.92,
    pros_tag: 'лучший баланс под ваш бюджет и задачи',
    build: verified,
    pros: pc.pros,
    cons: pc.cons,
  };
...
function mapVariantsToSuggestions(variants, budgetLabel) {
  return variants.map((v) => ({
    name: v.label,
    description: `${v.pros_tag.charAt(0).toUpperCase() + v.pros_tag.slice(1)}. Итого ≈ ${v.build.totalPrice.toLocaleString('ru-RU')} руб.${budgetLabel ? ` из ${budgetLabel.toLocaleString('ru-RU')} руб.` : ''}`,
    pros: v.pros,
    cons: v.cons,
...
• Если пользователь указал конкретный бюджет (сумма в рублях) и НЕ просил «несколько вариантов» / «2–3 варианта» / «альтернативы» — верни РОВНО 1 сборку (самую подходящую под запрос, ближе к 90–100% бюджета).
• Если пользователь явно просит несколько вариантов или альтернативы — 2 или 3 варианта (бюджетная / оптимальная / топовая).
...
async function smartFallback(pool, userMessage, normPre, workload) {
  ...
  const variantCount = resolveSuggestionVariantCount(budgetRub, userMessage, norm.normalized);
  if (fallbackBudget >= 50000) {
    if (variantCount >= 2) {
      ...
      if (variants.length > 0) {
        return { suggestions: mapVariantsToSuggestions(variants, budgetRub > 0 ? budgetLabel : null) };
      }
    } else {
      const one = generateSingleOptimalVariant(catalog, fallbackBudget, gpuPrefer, cpuHint, wl, effective);
      if (one) {
        return { suggestions: mapVariantsToSuggestions([one], budgetRub > 0 ? budgetLabel : null) };
      }
    }
  }
...
async function getAiResponse(pool, userMessage, buildSummary = null, history = null) {
  ...
  if (raw && typeof raw === 'string') {
    ...
    if (parsed && parsed.suggestions && Array.isArray(parsed.suggestions) && parsed.suggestions.length > 0) {
      ...
      const variantCount = resolveSuggestionVariantCount(budgetRub, clean, norm.normalized);
      if (!hasAttachedBuild && budgetRub >= 50000) {
        if (variantCount >= 2) {
          ...
          if (variants.length >= 2) {
            return { suggestions: mapVariantsToSuggestions(variants, budgetRub) };
          }
        } else {
          const one = generateSingleOptimalVariant(catalog, budgetRub, gpuPrefer, cpuHint, workload, effective);
          if (one) {
            return { suggestions: mapVariantsToSuggestions([one], budgetRub) };
          }
        }
      }

      const maxSuggestions = variantCount >= 2 ? variantCount : 1;
      const out = [];
      for (const s of parsed.suggestions.slice(0, maxSuggestions)) {
        let fixed = null;
